...
class StorageCostCalculatorTool(BaseTool):
    ...
    def _run(self, capacity_tb: float, dedup_ratio: float = 2.0,
             hot_percent: float = 20.0, growth_rate_percent: float = 15.0,
             workload_profile: str = "") -> Dict[str, Any]:
        ...
        pricing = {
            "AWS_S3": {"storage_gb": 0.023, "egress_gb": 0.09, "type": "Object Storage"},
            "Azure_Blob": {"storage_gb": 0.0184, "egress_gb": 0.10, "type": "Object Storage"},
            "Google_Cloud": {"storage_gb": 0.020, "egress_gb": 0.08, "type": "Object Storage"},
            "CVO": {"storage_gb": 0.060, "egress_gb": 0.09, "type": "NetApp Managed File"},
            "FSx_for_NetApp_ONTAP": {"storage_gb": 0.045, "egress_gb": 0.09, "type": "NetApp Managed File"},
            "Azure_NetApp_Files_Standard": {"storage_gb": 0.10, "egress_gb": 0.10, "type": "NetApp Managed File"},
        }

        results = {}
        for provider, rates in pricing.items():
            monthly_storage = effective_tb * 1024 * rates["storage_gb"]
            monthly_egress = effective_tb * 1024 * (hot_percent / 100) * rates["egress_gb"]
        ...
        needs_file_protocol = any(word in lower_profile for word in ["nfs", "smb", "file share", "file protocol", "ontap", "cifs"])

        if needs_file_protocol:
            # Prioritize NetApp-managed services
            file_options = {k: v for k, v in results.items() if v["type"] == "NetApp Managed File"}
            best = min(file_options, key=lambda p: file_options[p]["three_year_tco_usd"])
